...
%   **** NOT THE OUTPUT. SEE TOWARD END OF FILE FOR ACTUAL OUTPUT *****
...
max_zo_wind=25;     %keep roughness length (and Cd) constant beyond this wind speed (m/s)
                    %set equal to average of zo for (max_zo_wind-2)<wind<(max_zo_wind+2)
...
for i=1:nits
    ...
    %-------
    zo(u>max_zo_wind)=nanmean(zo(u>=(max_zo_wind-2)&u<=(max_zo_wind+2)));
    if length(zo)==1 & u>max_zo_wind
        if isnan(zo)
            zo=0.0067;
        end
    end
